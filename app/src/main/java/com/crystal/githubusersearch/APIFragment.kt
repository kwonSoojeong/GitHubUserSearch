...
import com.crystal.githubusersearch.databinding.FragmentApiBinding


class APIFragment : Fragment(R.layout.fragment_api){
    private var binding: FragmentApiBinding? = null
    private lateinit var listAdapter: UserListAdapter


    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        val fragmentHomeBinding= FragmentApiBinding.bind(view)
        binding = fragmentHomeBinding
        listAdapter = UserListAdapter()
        binding?.apply{
            apiRecyclerView.layoutManager = LinearLayoutManager(context)
            apiRecyclerView.adapter = listAdapter
        }
...
//        저장한 데이터 들 가져와서 UserList 로 만들어서 넣기
        listAdapter.submitList(listOf(User("", "dfasdf")))
    }
}